...
We first show that the colimit is [hProp].
  ...
  Proof.
    ...
    apply ap. simpl. apply (cp (f := fst) (g := snd) (x,y)).
    apply pp.
  Defined.
  ...
  Proof.
    ...
    apply ap. exact (@cp ((D j)*(D j)) (D j) fst snd (d,y)).
    apply pp.
  Defined.
  ...
  Proof.
    ...
    - rewrite (i_fr_step H' (diagram1 D (i:=i) (j:=i.+1) 1 x)).
      ...
      rewrite <- (ap_i_ap_f m (HPropQ_fun_ij_map (le_pred i.+1 m.+1 (le_S i.+1 m H')) H' x)).
    ...
    - intros j i0 f y; destruct f; cbn.
      ...
      + assert (rew : Peano.le_S i j l0 = l) by apply (path_ishprop (H := IsHProp_le _ _)); destruct rew.
        ...
        |[|- _ = (pp _ _ _ _ _ ?xx) @ ap _ ?ppp @ (pp _ _ _ _ _ ?yy)^] => 
         etransitivity; [| exact (ap_i_ap_f (j.+1) (x:= xx) (y:= yy) ppp)]
        end.
        apply ap_i_eq_ap_i_same.
      + destruct (leS_ge l l0).
        destruct p.
        ...
        apply ap_i_eq_ap_i_same.
      + destruct (not_ge_lt l0 l).
      + unfold eq_same; simpl.
        apply moveR_pM. apply moveR_pV.
        assert (X0 : (ap colim (HPropQ_fun_ij_map l0 l y)) = (HPropQ_colim_ij l0 y)^ @ (pp Gpd_graph_ D j j.+1 1 y)^ @ HPropQ_colim_ij l (coeq y) ).
        ...
        apply moveR_pM.
        match goal with
        |[|- _ = _ @ (_ @ ap colim ?ff) @ _ ] => pose (p := ap_i_ap_f i ff)
        end. simpl in p.
